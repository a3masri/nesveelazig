        ...
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          {(title || subtitle) && (
            <div className="min-w-0 border-l pl-2" style={{ borderColor: 'var(--border)' }}>
              {title && (
                <h1 className="font-display text-sm sm:text-base font-bold truncate uppercase" style={{ color: 'var(--text-primary)' }}>
                  {title}
                </h1>
              )}
              {subtitle && (
          ...
          {!isAdmin && (
            <>
              <button
                type="button"
                onClick={toggle}
                className="btn-duo btn-duo-ghost p-2.5 min-w-[40px] min-h-[40px] rounded-xl hidden sm:flex items-center justify-center"
              ...
              >
                ...
                <span className="font-bold text-xs text-cr-gold">{(profile?.total_points ?? 0).toLocaleString('tr-TR')}</span>
              </div>
              <Link
                to="/profile"
                className="btn-duo btn-duo-ghost p-2.5 min-w-[40px] min-h-[40px] rounded-xl flex items-center justify-center"
